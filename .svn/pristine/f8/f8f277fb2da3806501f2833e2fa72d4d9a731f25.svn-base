...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        StationManager instance =  StationManager.getInstance();

        for(ArrivalBoard next: instance.getSelected()){
            if ((next.getTravelDirn().equals(travelDirn)) && (next.getLine().getId().equals(lineId)))
                for (Arrival next2 : next) {
                    arrivals.add(next2);
                }
